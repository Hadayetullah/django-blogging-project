...
def pass_change(request):
    current_user = request.user
    changed = False
    # For PasswordChangeForm 'instance' is not allowed
    form = PasswordChangeForm(current_user)
    if request.method == 'POST':
        # form = PasswordChangeForm(request.POST, current_user)
        # form = PasswordChangeForm(current_user, request.POST)
        # Above two lines are also correct
        # But abmong two positional arguments if one of them is keyword argument then that argument position must place last
